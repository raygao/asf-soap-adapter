...
4. Salesforce Object lookup utility, provide a Salesforce object Id, and figure out if it is an Account, User, Lead, etc. <em>Salesforce::SfUtility.determine_sf_object_type(id)</em>. See the document for other useful method in this class.
5. Ability to call non-traditional SQLs. e.g. with "LIMIT", "GROUP BY", "HAVING", and "WITH DATA CATEGORY". example:
...
== Significant Changes
1. Detach hardcoded RForce 0.4.1
2. Conveninence classes, Salesforce::Account, User, ....
3. Chatter Feed
4. Object Lookup Utility (SF id -> SF object type)
5. Nontraditional query support
...
== Additional Note:
The enclosed the test app (asf-soap-adapter-rails-app) shows how to use the framework. Go into its test/unit, where examples are provided.

I am currently, updating my reference app SFRWatcher to SFRWatcher_v20 to match the switch from the old gem to this new gem. Once it is ready, it will be posted on the web soon.
